...
robot: ArticulationCfg = ArticulationCfg(
    ...
    init_state=ArticulationCfg.InitialStateCfg(
        pos=(0.25, -0.25, 1.2), # 初始化位置，z值过低会发生碰撞改变方向
    ),
    actuators={
...
camera_cfg = CameraCfg(
    prim_path="{ENV_REGEX_NS}/Robot/Origin_.*/CameraSensor",
    update_period=0,  # 每个sim_dt更新相机（不是保存）
    height=480,
    width=640,
    data_types=[
        "rgb",
    ...
    spawn=sim_utils.PinholeCameraCfg(
        focal_length=24.0,          # 焦距，越大视野越窄
        focus_distance=400.0,    # 对焦距离
        horizontal_aperture=20.955,  # 水平视野大小（单位是毫米）
        clipping_range=(0.1, 1.0e5)  # 可见距离范围：从0.1米到10万米
    ),
    offset=CameraCfg.OffsetCfg(pos=(0.004, 0.004, 0.0), rot=(0.0, 1.0, 0.0, 0.0), convention="ros"),
)
# rot=(0.5, -0.5, 0.5, -0.5)
...
class NewRobotsSceneCfg(InteractiveSceneCfg):
    ...
    # prim_utils.create_prim("{ENV_REGEX_NS}/Robot/Origin_00", "Xform")
    # prim_utils.create_prim("{ENV_REGEX_NS}/Robot/Origin_01", "Xform")
    camera = camera_cfg.replace(prim_path="{ENV_REGEX_NS}/Robot/body/CameraSensor")
    



def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    sim_dt = sim.get_physics_dt()  # 60帧
    sim_time = 0.0
    count = 0
    ...
    output_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "output", "camera") # 输出保存路径是：当前脚本同目录下的 output/camera/ 文件夹
    rep_writer = rep.BasicWriter(       # 使用 Omniverse Replicator 的 BasicWriter 来保存图像数据（如 RGB、分割图等）。它只支持保存为 .npy 格式，所以后面有数据转换。
    ...
    while simulation_app.is_running():
        ...
        forces[:, :, 2] = robot_mass * gravity * 1.05 / 4
        moment[:, 0, 2] = 0.005
        ...
        if args_cli.save and count % 30 == 0:    # 每秒保存一次
            # Save images from camera at camera_index
            # note: BasicWriter only supports saving data in numpy format, so we need to convert the data to numpy.
            single_cam_data = convert_dict_to_backend(
                {k: v[camera_index] for k, v in camera.data.output.items()}, backend="numpy"   # 把相机 torch 张量格式转换为 numpy，便于 Replicator 写入磁盘。
            )
            ...
            rep_output = {"annotators": {}}  # Replicator 需要特定的格式，我们逐个键值打包数据。
